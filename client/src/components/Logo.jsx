...
export default function Logo({ className = "", light = false }) {
  ...
  return (
    <Link to="/" className={`text-logo ${className}`} aria-label="Kamal Waheed home">
      <span className={`text-logo__icon ${light ? "text-white" : "text-secondary"}`}>
        <span className="material-symbols-outlined" style={{ fontSize: "32px" }}>home_living</span>
      </span>
...
export function LogoFull({ className = "" }) {
  ...
  return (
    <span className={`text-logo ${className}`}>
      <span className="text-logo__icon text-secondary">
        <span className="material-symbols-outlined" style={{ fontSize: "32px" }}>home_living</span>
      </span>
