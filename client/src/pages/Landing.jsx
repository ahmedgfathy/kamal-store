...
export default function Landing() {
  ...
  useEffect(() => { api.get("/products/featured").then(r => setFeatured(r.data?.products || [])).catch(() => setFeatured([])); }, []);

  return (
    <div className="store-home">
      <section className="new-hero">
        <div className="new-hero__image">
          <img src="https://images.unsplash.com/photo-1556909212-d5b604d0c90d?w=1400&q=90" alt="Kamal Waheed home collection" />
          <div className="new-hero__badge"><strong>500+</strong><span>{lang === "ar" ? "قطعة مختارة" : "curated pieces"}</span></div>
        </div>
        <div className="new-hero__content">
          <p className="eyebrow">KAMAL WAHEED · 2026</p>
          <h1>{lang === "ar" ? <>منزلٌ يشبهك،<br/><em>بتفاصيل أجمل.</em></> : <>A home that feels<br/><em>uniquely yours.</em></>}</h1>
          <p className="new-hero__copy">{lang === "ar" ? "تشكيلة عصرية من أدوات المطبخ والديكور والمنسوجات، مختارة لتضيف جمالاً عملياً لكل يوم." : "Modern kitchenware, décor and textiles selected to bring practical beauty into every day."}</p>
          <div className="new-hero__actions">
            <Link to="/products" className="button-primary">{t("landing.viewAll")} <span>←</span></Link>
            <Link to="/products?category=kitchen" className="button-link">{lang === "ar" ? "اكتشف المطبخ" : "Explore kitchen"}</Link>
          </div>
      ...
      <section className="editorial-banner page-shell">
        ...
        <img src="https://images.unsplash.com/photo-1556912167-f556f1f39fdf?w=1200&q=90" alt="Modern kitchen collection" />
      </section>
